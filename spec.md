...
erDiagram
    CONFIG_SETTINGS {
        ...
        string folder_unadjusted_price "未調整股價(日)EXCEL資料夾【第一優先：收盤價G欄、漲跌幅J欄】"
        string folder_daily_price "日均價DATA EXCEL資料夾【第二優先 Fallback】"
...
> [!IMPORTANT]
> **「未調整股價(日)」第一優先設計**：收盤價（現股股價）與今日漲跌幅（報酬率）的檢索優先順序已提升。系統優先從 `folder_unadjusted_price`（`C:\TejPro\TejPro\DataExport\未調整股價(日)`）資料夾下最新 Excel 的 **G 欄「收盤價(元)」**與 **J 欄「報酬率％」**取得精確數據；只有在此來源找不到個股資料時，才 Fallback 至原有的「日均價 DATA」資料夾，最後才走網路搜尋路線。

```mermaid
flowchart TD
    A[開始取得收盤價與漲跌幅] --> B[第一優先: 檢索「未調整股價-日」資料夾最新 Excel]
    B --> C{按代號/名稱找到該股?}
    C -->|是| D[精確讀取 G欄收盤價 + J欄報酬率％]
    D --> E[標記 來源: 未調整股價-日]
    C -->|否| F[第二優先 Fallback: 檢索「日均價 DATA」資料夾最新 Excel]
    F --> G{找到該股資料?}
    G -->|是| H[模糊匹配收盤/均價欄位]
    H --> I[標記 來源: 本機資料庫]
    G -->|否| J[最終 Fallback: Yahoo Finance API 網路搜尋]
    J --> K{網路請求成功?}
    K -->|是| L[載入網路最新數據]
    K -->|否| M[優雅降級: 載入金融估算預設數據]
    L --> N[標記 來源: 網路搜尋]
    M --> N
    E --> O[渲染 PDF 籌碼與均價核心數據卡片]
    I --> O
    N --> O
    O --> P[PDF 報告生成完畢]
```
...
| 數據維度 | 設定資料夾 (持久化) | 欄位說明 / 匹配方式 | 網路搜尋 Fallback 來源 | PDF 來源標記 |
|---------|------------------|----------------------|----------------------|-------------|
| **收盤價 (第一優先)** | **未調整股價(日) EXCEL 資料夾** | **G 欄「收盤價(元)」精確讀取** | — (Fallback 至下方) | `[來源: 未調整股價-日]` |
| **漲跌幅 (第一優先)** | **未調整股價(日) EXCEL 資料夾** | **J 欄「報酬率％」精確讀取** | — (Fallback 至下方) | `[來源: 未調整股價-日]` |
| **日均股價 (第二優先)** | 日均價 DATA EXCEL 資料夾 | `["未調整收盤價", "均價", "收盤", "價格", "均"]` 模糊匹配 | Yahoo Finance Chart API | `[來源: 本機資料庫]` / `[來源: 網路搜尋]` |
