...
| **2. 價內外程度** | 價內外 | 優先讀取 `"價內外"` 欄位文字（如 `"價外 5.3%"`），若為數值則利用 `(標的價格 - 履約價) / 履約價 * 100` 自動計算，呈現 `"價內/價外 X%"`。 |
...
為了讓 PDF 報告具有最佳的排版（防範分頁溢出），綜合大評比表格最多**限制顯示 10 筆最優權證**。
* **排序規則**：依據權證的 `"推薦評分"` 欄位（滿分 100 分，綜合 Delta、天期、槓桿、IV/HV、成交量權重）進行降序排列，選取前 10 筆最優標的。
